...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Admin Sign In — Pushpa Online Tuition" },
...
function AuthPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signin") {
        ...
        if (error) throw error;
        toast.success("Account created. Ask an existing admin to grant admin access, then sign in.");
        setMode("signin");
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-[calc(100vh-8rem)] flex items-center justify-center px-4 py-12 bg-secondary/40">
      <div className="w-full max-w-md rounded-2xl border border-border bg-background p-8 shadow-sm">
        <Link to="/" className="flex items-center gap-2.5 mb-6">
          ...
          <span className="font-serif text-lg font-semibold">Pushpa<span className="text-accent">.</span></span>
        </Link>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {mode === "signin"
            ? "Access the tutor applications dashboard."
            : "New accounts require an admin role grant before dashboard access."}
        </p>
